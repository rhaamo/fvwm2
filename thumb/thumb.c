...
int main(int argc, char **argv) {
	...
	thumbheight = (int)((float)windowattr.height /
			((float)windowattr.width/(float)thumbwidth));
	...
	if (4*thumbwidth >= windowattr.width ||
		...
		image = imlib_create_cropped_scaled_image(0, 0,
				windowattr.width, windowattr.height,
				thumbwidth, thumbheight);
	} else {
		image = imlib_create_scaled_image_from_drawable((Pixmap)0, 0, 0,
				windowattr.width, windowattr.height,
				4*thumbwidth, 4*thumbheight, 1, 1);
		imlib_context_set_image(image);
		image = imlib_create_cropped_scaled_image(0, 0,
				4*thumbwidth, 4*thumbheight,
				thumbwidth, thumbheight);
	}
